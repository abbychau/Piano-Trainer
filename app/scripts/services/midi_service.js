...
export default class MidiService {
  ...
  setKeyNumber(keyNumber, intensity) {
    const wasPressed = !!this.currentInputState[keyNumber];
    const isPressing = intensity > 0;
    const now = performance.now ? performance.now() : Date.now();
    const lastNoteOffAt = this.lastNoteOffAtByKey[keyNumber] || 0;
    const wasLikelyReleaseEcho = now - lastNoteOffAt < MidiService.noteOnAfterNoteOffGuardMs;
    if (!wasPressed && isPressing && !wasLikelyReleaseEcho) {
      this.noteOnCallback(keyNumber, intensity);
    }
  ...
  onMidiMessage(msg) {
    ...
    const [status, keyNumber, velocity] = msg.data;
    ...
    // MIDI allows Note On with velocity 0 to mean Note Off.
    const intensity = isNoteOn ? velocity : 0;
...
MidiService.noteOnAfterNoteOffGuardMs = 45;
