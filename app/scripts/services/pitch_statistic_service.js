...
class PitchStatisticService {
  ...
  getSuccessRateOfLast(n = 10) {
    const lastEvents = this.stats.slice(-n);
    if (lastEvents.length === 0) {
      return 0;
    }
    const successCount = _.filter(lastEvents, el => el.success).length;
    return successCount / lastEvents.length;
  }

  getTimingSuccessRateOfLast(n = 10) {
    const lastEvents = this.stats.slice(-n);
    if (lastEvents.length === 0) {
      return 0;
    }
    const timingSuccessCount = _.filter(lastEvents, el => {
      if (el.timingSuccess == null) {
        return !!el.success;
      }
      return !!el.timingSuccess;
    }).length;
    return timingSuccessCount / lastEvents.length;
  }
